 ...
 * 
 */
class LibrarytagBehavior extends ModelBehavior {
    /**
	...
	$Librarytag =& new Librarytag;
	$Librarytag =& new Librarytag;
        if ($model->hasField('metakey') 
		&& $Librarytag->hasField($this->settings[$model->name]['tag_label'])) {
		...
		foreach($tag_list as $t) {
			if ($res = $Librarytag->find($this->settings[$model->name]['tag_label'] . " LIKE '" . $t . "'")) {
				$tag_info[] = $res['Librarytag']['id'];
			} else {
				$Librarytag->save(array('id'=>'',$this->settings[$model->name]['tag_label']=>$t));
				
				//die(pr($this->settings[$model->name]['tag_label']));

				$tag_info[] = sprintf($Librarytag->getLastInsertID());
			}
			unset($res);
		}

		// This prepares the linking table data...
		$model->data['Librarytag']['Librarytag'] = $tag_info;
