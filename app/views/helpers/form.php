 ...
 */
	function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $selected = null, $attributes = array()) {
		App::import('Vendor', 'persiandate'); //Added By Arash
		$attributes += array('empty' => true);
		...
		if (!empty($selected)) {
			if (is_array($selected)) {
				...
				if (is_numeric($selected)) {
					//Edited By Arash
					$selected = persian_strftime_utf('%Y-%m-%d %H:%M:%S', $selected);
				}
				$meridian = 'am';
				$pos = strpos($selected, '-');
				if ($pos !== false) {
		...
		switch ($name) {
			...
			case 'month':
				if ($options['monthNames'] === true) {
					$data['01'] = __('فروردین', true);
					$data['02'] = __('اردیبهشت', true);
					$data['03'] = __('خرداد', true);
					$data['04'] = __('تیر', true);
					$data['05'] = __('مرداد', true);
					$data['06'] = __('شهریور', true);
					$data['07'] = __('مهر', true);
					$data['08'] = __('آبان', true);
					$data['09'] = __('آذر', true);
					$data['10'] = __('دی', true);
					$data['11'] = __('بهمن', true);
					$data['12'] = __('اسفند', true);
