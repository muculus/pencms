...
foreach ($submits[0]['Submit'] as $itemColumn){
    if($itemColumn['deleted'] == 0){
		$data[] = array(
			"id" => $itemColumn['id'], 
			"subject" => $itemColumn['subject'],
			"title" => $itemColumn['title'],
			"sourc_paper" => $itemColumn['author'],
...
$(document).ready(function() {
	jQuery("#gridTable").jqGrid({
		datatype: 'local',
		colNames:['id','subject', 'title', 'sourc_paper','file','publish','created'],
		colModel:[
			{name:'id',index:'id', width:'30'},
			{name:'subject',index:'subject', width:'50'},
			{name:'title',index:'title'},
			{name:'sourc_paper',index:'sourc_paper'},		
