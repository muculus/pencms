...
foreach ($groups[0]['Group'] as $itemColumn){
    if($itemColumn['deleted'] == 0){
		$data[] = array(
			"id" => $itemColumn['id'], 
			"name" => $itemColumn['name'],
			"parent_id" => $itemColumn['parent_id'],
			"created" => $itemColumn['created'],
			"modified" => $itemColumn['modified'],
...
$(document).ready(function() {
	jQuery("#gridTable").jqGrid({
		datatype: 'local',
		colNames:['id','name', 'parent_id', 'created','modified','publish','created', 'source', 'pages'],
		colModel:[
			{name:'id',index:'id', width:'30'},
			{name:'name',index:'widget_id', width:'50'},
			{name:'parent_id',index:'parent_id'},
			{name:'created',index:'created'},		
			{name:'modified',index:'modified', width:'40'},
