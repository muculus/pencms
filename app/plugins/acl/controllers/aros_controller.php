...
class ArosController extends AclAppController
{
	...
	function admin_check($run = null)
	{
		...
		$roles = $this->{$role_model_name}->find('all', array('order' => $role_display_field));
		...
		$users = $this->{$user_model_name}->find('all', array('order' => $user_display_field));
		...
		if(isset($run))
		{
			...
			if(count($missing_aros['users']) > 0)
			{
				foreach($missing_aros['users'] as $k => $user)
				{
					 ...
					 */
					$parent_id = $this->Aro->field('id', array('model' => $role_model_name, 'foreign_key' => $user[$user_model_name][Inflector::underscore(Configure :: read('acl.aro.role.model')) . '_id']));
	...
	function admin_update_user_role()
	{
        ...
        $data = array($user_model_name => array('id' => $this->params['named']['user'], Inflector::underscore(Configure :: read('acl.aro.role.model')) . '_id' => $this->params['named']['role']));
	...
	function admin_user_permissions($user_id = null)
	{
	    ...
	    else
	    {
            ...
            if(empty($user_aro))
            {
                ...
                $this->Session->setFlash(sprintf(__d('acl', "The user '%s' does not exist in the ARO table", true), $display_user[$user_model_name][$user_display_field]), 'flash_error');
            }
            else
            {
            	$actions = $this->AclReflector->get_all_actions();
